...
int count(Node*current){
			//we have to reach every node like display function
...
int count2(Node*current){
	...
	{
		int left=count2(current->left);
		int right=count2(current->right);
		return left+right+1;
		
		same as==return((count2(current->left)+count2(current->right))+1));
    }*/
...
//(curr!=NULL)
 ...
 int rangeSearch(int k1, int k2){
 	...
 	for(int i=k1;i<=k2;i++){
 		if(searchHelper(i)!=NULL){
 			cout<<"Add:  "<<searchHelper(i)<<"  Value:"<<i<<endl;
 			count++;
 		}
 	}
 	return count;
 ...
 int heighthelper(){
   ...
   //base case
        ...
        {
            int left=height(current->left);//height of the left subtree is calculated
            int right=height(current->right);//height of the right subtree is calculated
            
            if(left>right)  return (1+left);
            //and the maximum among them is returned after adding one to it
            else return (1+right);//for the current level
        }
}
